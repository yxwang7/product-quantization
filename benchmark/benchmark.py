...
if __name__ == '__main__':
    ...
    def evaluate_number_of_probes(number_of_probes):
        ...
        for (i, query) in enumerate(Q):
            if T[i] in query_object.get_candidates_with_duplicates(
                    query):
                score += 1
        return float(score) / len(Q)
    ...
    score = 0
    for (i, query) in enumerate(Q):
        if query_object.find_nearest_neighbor(query) == T[i]:
            score += 1
    t2 = timeit.default_timer()

    print('Query time: {}'.format((t2 - t1) / len(Q)))
    print('Precision: {}'.format(float(score) / len(Q)))